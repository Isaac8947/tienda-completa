...
// Si el full_name está vacío pero existe username, usar username
if (empty($adminData['name']) || $adminData['name'] === 'Administrador') {
    $adminData['name'] = $adminData['username'] ?? 'Administrador';
}
...
function getDashboardStats() {
    ...
    try {
        ...
        $stmt = $db->query("SELECT COUNT(*) as total_offers FROM offers");
        $stats['total_offers'] = $stmt->fetch()['total_offers'] ?? 0;
        
        $stmt = $db->query("SELECT COUNT(*) as active_offers FROM offers WHERE status = 'active' AND end_date > NOW()");
        $stats['active_offers'] = $stmt->fetch()['active_offers'] ?? 0;
        
        $stmt = $db->query("SELECT COUNT(*) as expired_offers FROM offers WHERE status = 'active' AND end_date <= NOW()");
        $stats['expired_offers'] = $stmt->fetch()['expired_offers'] ?? 0;
        
        // Oferta principal activa
        $stmt = $db->query("SELECT title, discount_percentage, end_date FROM offers 
                           WHERE status = 'active' AND is_main = 1 AND end_date > NOW() 
                           ORDER BY created_at DESC LIMIT 1");
        $stats['main_offer'] = $stmt->fetch();
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900"><?php echo number_format($stats['active_offers']); ?></p>
                                <div class="flex items-center mt-2">
                                    <?php if ($stats['main_offer']): ?>
                                        <i class="fas fa-fire text-red-500 text-sm mr-1"></i>
                                        <span class="text-red-600 text-sm font-medium">
                                            <?php echo $stats['main_offer']['discount_percentage']; ?>% principal
                                        </span>
                                    <?php else: ?>
                                        <span class="text-gray-500 text-sm">
                                            Sin oferta principal
                                        </span>
                                    <?php endif; ?>
                                </div>
                            </div>
